...
if ($busca_total->rowcount(PDO::FETCH_ASSOC) > 0) {
	...
	while ($row = $busca_total->fetch(PDO::FETCH_ASSOC)) {
		...
		echo " <tr>
              ...
              <td>".$row['idade']."</td>	
              <td><input type='button' onclick=\"location.href='criarrelatoriopsi.php?idut=".$row['idutente']."&voltar=".$ur=end(explode("/", $_SERVER['PHP_SELF']))."'\"value='Criar' class='btn btn-primary'></td>
              <tr>";
