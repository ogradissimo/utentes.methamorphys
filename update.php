...
if($row['id_tipo_control']==3)
{
		...
		$nomecheck[]= $string;
		//echo "INSERT INTO ".$row2['nome_tabela']." (id_utente,".$row2['nome_campo']." ) values ( ? , ?)";
		//echo "DELETE  FROM ".$row2['nome_tabela']." WHERE id_utente = ?";
		$array3[]="DELETE  FROM ".$row2['nome_tabela']." WHERE id_utente = ?";
...
if($campos != '')
{
 ...
 $values=substr($values, 0, -2);
  ...
  $STH2 = $DBH->prepare("UPDATE ".$tabela." SET ".$campos." Where idutente = ".$utente."");
...
for($e=0;$e<$contagem;$e++)
{
	$opcoes = $_GET[$nomecheck[$e]];
	 		...
	 		$STH4 = $DBH->prepare($array3[$e]);
			$STH4->bindParam(1,$utente);
			$STH4->execute();
	if(count($opcoes)!= 0)
{
 ...
 $url=$_GET['url'];
  if($count == 0 or $count2==1)
{
		echo '<meta http-equiv="refresh" content="0; url='.$url.'?idut='.$utente.'&alt=true" />';
