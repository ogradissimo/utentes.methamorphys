...
$calcula_total = $DBH->prepare("SELECT * FROM mensagens Where id_enviado=".$_SESSION['idut']."");
$calcula_total->execute();
$busca_total = $DBH->prepare("SELECT utilizador.utilizador, mensagens.id_mensagem ,mensagens.mensagem , mensagens.hora,  mensagens.data FROM mensagens, utilizador Where mensagens.id_destino=utilizador.id_ut and id_enviado=".$_SESSION['idut'] ." ORDER BY id_mensagem DESC LIMIT " . $inicio . ", " . $limite . "");
$busca_total->execute(); 
...
echo'                      
                        <tr>
                          <td class="mailbox-name"><a href="lermensagem.php?idmen='.$row['id_mensagem'].'&env=true">'.$row['utilizador'].'</a></td>
